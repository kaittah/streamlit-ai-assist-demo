...
def create_new_graphing_pr(repo_name: str, repo_owner: str, token:str, graph_title: str, graphing_file_full_path:str, new_code: str):

    branch_name = graph_title.strip().replace(' ', '-').lower() + f'-{random.randrange(1000, 9999)}'

    create_new_branch(token=token, branch_name=branch_name, repo_name=repo_name, repo_owner=repo_owner)
    ...
    new_code = '\n\n' + new_code
    repo.update_file(path=contents.path, message="Add graphing function", content=decoded_content + new_code, sha=contents.sha, branch=branch_name)
    repo.create_pull(
        base="main", 
        head=branch_name,
